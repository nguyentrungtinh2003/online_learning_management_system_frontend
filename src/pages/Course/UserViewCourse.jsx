...
import { checkEnrollment } from "../../services/courseapi";
...
export default function UserViewCourse() {
  ...
  useEffect(() => {
    ...
    const courseId = parseInt(id); // id từ useParams

    if (userId && courseId) {
      checkEnrollment(userId, courseId).then((enrolled) =>
        setHasEnrolled(enrolled)
      );
    }
  }, [id]); // chỉ cần phụ thuộc vào id
  ...
  const buyCourse = async (id) => {
    ...
    try {
      ...
      if (result.statusCode === 200) {
        toast.success("🎉 Mua khoá học thành công!", { autoClose: 500 });
        setHasEnrolled(true);
        setBuyLoading(false);
      } else {
          ...
          <div>
            ...
            <div className="flex items-center gap-6 ">
              <button
                onClick={() =>
                  hasEnrolled
                    ? navigate(`/user-course/view-lesson/${id}`)
                    : buyCourse(id)
                }
                disabled={buyLoading}
                className="px-6 py-3 font-bold rounded-xl transition-all duration-300 flex items-center justify-center gap-2 min-w-[200px]
    bg-yellow-400 text-black hover:scale-105 hover:shadow-lg"
          ...
          <div className="mt-6 text-center">
            ...
            >
              {buyLoading ? (
                ...
                t("Start Learning") // ví dụ: "Bắt đầu học"
              ) : course?.price > 0 ? (
                t("course.enrollNow", { price: course.price }) // ví dụ: "Mua khoá học 200 xu"
              ) : (
                t("course.learnNowFree") // ví dụ: "Học ngay miễn phí"
              )}
            </button>
          </div>
        </div>
      ...
      <section className="py-10 bg-wcolor dark:bg-darkSubbackground">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="bg-wcolor dark:bg-darkSubbackground dark:border dark:border-darkBorder rounded-2xl shadow-lg p-4 md:p-10 space-y-6">
            {lessonsToDisplay.map((lesson, index) => (
              ...
              >
                <div className="relative px-4 py-2 w-full rounded-lg hover:bg-tcolor dark:hover:bg-darkHover">
                  <p className="text-lg font-semibold dark:text-darkText">
                    {lesson.lessonName}
                  </p>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 px-6">
          ...
          <div>
            <h3 className="text-2xl font-bold mb-4">
              {t("course.testimonialTitle")}
            </h3>
            <blockquote className="bg-wcolor dark:bg-darkHover p-6 rounded-xl shadow">
              <p className="text-lg italic">{t("course.testimonial")}</p>
      ...
      <section className="py-16 bg-gradient-to-r from-indigo-700 to-purple-800 text-white">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <p className="mb-8">{t("course.readyDesc")}</p>
          ...
          >
            {buyLoading ? (
              ...
              t("course.buyCourse", { price: course.price }) // ví dụ: "Mua khoá học 200 xu"
            ) : (
              t("course.startFree") // ví dụ: "Học miễn phí"
            )}
          </button>
        </div>
      </section>
